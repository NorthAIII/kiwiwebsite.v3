...
| # | Task | Durum | Açıklama |
|---|------|-------|----------|
| 7.01 | TASK-7.01 | ⬜ Bekliyor | Umami bileşeni (`umami-script.tsx` YENİ) + `[locale]/layout.tsx` head entegrasyonu + izole render testi (`vi.mock("next/script")`) |
...
**Son Güncelleme:** 2026-07-01 — plan-phase 7: 2 task yazıldı (7.01 bileşen+layout+izole render testi; 7.02 before/after perf regresyon doğrulaması). Canlı +1 doğrulaması kod task'ı değil → verify-phase (merge-sonrası). Sıradaki adım: verify-plan 7.

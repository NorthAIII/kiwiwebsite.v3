...
**Son Güncelleme:** 2026-07-01 — run-task 7.02 ✅: Umami sonrası before/after Lighthouse — **regresyon YOK** (same-env mobil TR `/`: before perf 90/LCP 3009ms → after 88/2714ms, dağılımlar örtüşük; masaüstü 100/660ms; CLS 0). `afterInteractive` script LCP penceresinden sonra yükleniyor (network-requests audit'i isteğin fiilen alındığını doğruladı) → LCP'ye zarar yok. preconnect eklenmedi (YAGNI); DECISIONS'a girdi gerekmedi. Artefaktlar `docs/perf/*-faz7.*`. Fazdaki tüm task'lar bitti → sıradaki: verify-phase 7 (canlı +1, merge sonrası).

...
**Son Güncelleme:** 2026-06-29 — run-task TASK-3.09 ✅ (S8 adversarial/holistik) — **fazın son senaryo task'ı, 9/9 ✅**. Kanonik fresh-prod-serve (`rm -rf .next && next build` exit 0, 0-uyarı, 37 statik sayfa = S8-build regresyon tabanı re-teyit; fresh listening-PID 30537 teyit + ground-truth title eşleşmesi, iş sonu kill; stray PID 12267 portsuz/dokunulmadı). Araç: curl raw HTML (JS-off SSG) + Playwright MCP `browser_run_code_unsafe` (race). **AG1 JS-off SSG (TR+AR):** tüm bölüm ID (#how/#sectors/#bunker/#forum/#chat) + `<h1>/<h2>` başlıklar + gövde metinleri + nav/CTA (hero ikincil "İşleyen örnekleri gör"→#sectors) raw HTML'de okunur; `<canvas>`=0 (Living Flow client-only, beklenen; base-wash wrapper SSR'da); AR `dir=rtl` + 0 MISSING_MESSAGE; kritik içerik client-only'ye gömülü değil. **AG2 toggle race:** tema 7-UI-tıklama + JS senkron burst(8,9) → `html.dark=localStorage=aria-pressed=icon` tutarlı (3/3, React batch yarışı yenildi); dil senkron burst(es→de→ar tek tick) son(ar) temiz kazanır (url=lang=dir=langBtn) + hızlı ardışık(en→ar→de→es→tr,180ms) son(tr) tutarlı — yarım-durum yok. **AG3 scroll/anchor race:** ardışık(230ms)/rAF(~16ms)/80ms zıplama son hedefe(#forum) tam iner (forumTop≈0), tek anchor isabetli; tüm sayfa kaydırılınca 30/30 `[data-reveal]` görünür, **0 takılı**; #top dönüş scrollY=0 (scroll fonksiyonel); tüm koşular 0 konsol hatası. **AG4 triyaj (TK6):** kapsam-içi bug YOK, kaynak kod değişmedi. Record-not-fix: aynı-JS-tick(0ms) anchor burst Lenis'i ara konuma çözer (≥16ms arayla temiz iner, takılma/kırılma yok) — fiziksel imkânsız girdi, kullanıcı etkisi yok, Lenis tek-tick çoklu-scrollTo iç davranışı (app/ScrollTrigger bug'ı değil). Sıradaki adım: UAT (verify-phase 3). Detay: archive/TASK-3.09.md + git log.

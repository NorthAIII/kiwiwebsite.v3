...
**Son Güncelleme:** 2026-06-29 — run-task TASK-3.07 ✅ (S4 kontroller & kalıcılık). Kanonik fresh-prod-serve: `rm -rf .next && next build` exit 0 **temiz** (0 hata/uyarı, 37 statik sayfa → S8-build re-teyit) + port 3100, fresh listening-PID 3357729 teyit (Ready 326ms; Jun28 stray 12708 portu tutmuyordu, dokunulmadı), iş sonu kendi process kill. Araç: Playwright MCP `browser_run_code_unsafe` (click/keyboard/Escape/reload/localStorage + `getComputedStyle` outline + sentinel `MutationObserver` enjeksiyonu, atomik script + try/catch). **Sonuçlar:** (1) **Tema** light(html.dark=false/ls=null/canvas=1/GL-alive) → toggle dark(html.dark=true/ls="dark"/canvas+GL canlı) → **reload kalıcı** (html.dark=true/ls="dark"; FOUC inline script `<head>`'de `<body>` öncesi) → geri-toggle light(ls="light"); **Living Flow uniform** = FlowCanvas'ın MutationObserver'ı ([FlowCanvas.tsx:256-269](../../src/components/living-flow/FlowCanvas.tsx#L256)) ile birebir aynı sentinel observer (`documentElement` `attributeFilter:['class']`) toggle'da fired=1 + lastDark doğru → canlı güncelleme yolu çalışıyor (canvas/GL toggle boyunca canlı → gerçek observer bağlı; preserveDrawingBuffer yok [FlowCanvas.tsx:385](../../src/components/living-flow/FlowCanvas.tsx#L385) → THREE uniform dış-okunamaz, mekanizma-proxy en dürüst yol). (2) **Dil-switcher** `aria-haspopup="listbox"`+`aria-expanded` false↔true; 5 dil (Türkçe/English/العربية/Deutsch/Español), aktif `aria-selected`+`•`; **Escape kapatır**, **dış-tık kapatır**; klavye: trigger focus → **Enter açar** → **Tab option'a girer** (role=option/focusable button); **path korunur** (home `/en`→`/de`; subpage `/en/spor-salonu-yazilimi`→`/es/spor-salonu-yazilimi`); triggerCount=2 (Nav+Footer). (3) **Klavye-only + focus-visible** Tab ile 16 ardışık durak (logo→4 nav link→tema→dil→nav CTA→2 hero CTA→Alpfit/Crew OS kart→4 sektör tab), **16/16 distinct = focus-trap yok**; 550ms settle sonrası **16/16 outline `2px solid rgb(31,122,61)` offset 3px** (= `--color-green` light, M1 F1.4); görsel kanıt (lang button yeşil outline). İlk 120ms ölçümde RGB/offset sapması = Tailwind `transition-colors` outline-color'ı anime ediyor (mid-transition artefaktı), settle ile giderildi. (4) Tüm etkileşimlerde konsol **0 error / 0 warning**. **Triyaj (TK6):** kapsam-içi düzeltilecek bug YOK. **Bulgu — anchor-drop (record-not-fix, kullanıcı onayı 2026-06-29):** dil değişiminde path korunuyor ama URL anchor düşüyor (`/en#sectors`→Almanca→`/de` hash="" scrollY=0); kök neden [LanguageSwitcher.tsx:50](../../src/components/LanguageSwitcher.tsx#L50) `router.replace(pathname,{locale})` — next-intl `pathname` hash içermez; M3 F3.4 resmi kabul kriteri yalnız **path** ister (✓ geçiyor), "anchor koru" yalnız S4 senaryo dokümanında; düşük şiddet → kullanıcı record-not-fix+ertele seçti, fix-task açılmadı. **Bilinen aria-mismatch (TK6):** dil-switcher `aria-controls`/roving-tabindex/`aria-activedescendant` eksik (a11y 89, DECISIONS 2026-06-28) → record-not-fix. Kaynak kod değişmedi. Adım: run-task (TASK-3.08). Kanonik fresh-prod-serve: `rm -rf .next && next build` exit 0 **temiz** (0 hata/uyarı, 37 statik sayfa → S8-build regresyon tabanı re-teyit) + port 3100, fresh listening-PID 3303870 teyit (Jun28 stray 12708 portu tutmuyordu, dokunulmadı), iş sonu kendi process kill. Araç: Playwright MCP `browser_run_code_unsafe` (raw `page.emulateMedia` + `context.addInitScript` + izole `browser.newContext` — MCP'de ayrı emulateMedia tool'u yok, escape-hatch ile çözüldü; TK5 kod-inceleme fallback'ine gerek kalmadı). Headless'ta WebGL gerçek çalışıyor (baseline canvas=1, StaticFlow yok) → high/low yolu gerçekten koşuldu, fallback değil. **Sonuçlar:** (1) tema light isDark=false/bodyBg rgb(247,246,241), dark(localStorage) isDark domcontentloaded'da true + inline tema script head'de body'den önce = **FOUC yok**, iki tema da canvas=1; (2) reduced-motion → canvas=0 + StaticFlow 4-path + Reveal 12/12 görünür (M3 F3.1), 0-error; (3) no-WebGL (getContext shim → supportsWebGL=false) → canvas=0 + StaticFlow, 0-error (motion açıkken `[data-reveal]` scroll'da açılır: before 0/30→after 30/30, takılmıyor); (4) mobil 390 → mqMobile=true + canvas=1 ⟹ **low** (gating-değişmezi: ≤768px'te kod "high" üretemez); (5) **AR-RTL×dark×reduced birlikte**: dir=rtl/lang=ar/isDark=true, canvas=0+StaticFlow, StaticFlow color rgb(242,241,232)=LIGHT_INK (tema-duyarlı), yatay taşma yok, 0-error, nav/hero doğru aynalanmış (görsel); (6) responsive 320/768/1440 → hepsinde scrollWidth<innerWidth (yatay taşma yok, offender=0) + **CLS=0** (loadavg 0.49 ölçüm öncesi), canvas=1, 0-error. Triyaj (TK6): kapsam-içi yeni layout/taşma bug'ı YOK, CLS bilinen-green; bilinen a11y/perf açığı bu testte yüzeye çıkmadı (S3 degradasyon ekseni, Lighthouse koşulmadı) → sahipli/ertelenmiş. Gözlem: `/`→`/en` locale-detection (Accept-Language en-US) = next-intl beklenen, S1 kapsamı (TASK-3.02), S3 bulgusu değil. Kaynak kod değişmedi. Adım: run-task (TASK-3.07).

...
**Son Güncelleme:** 2026-07-05 — run-task TASK-14.08 ✅: S7 chatbot 0-token doğrulandı. **`next start`/`test:e2e` bu oturumda exit 144 ile anında öldü** (cloud-devcontainer sandbox worker-fork; Faz 13 riski aktif) → tarayıcı-server katmanı koşulamadı → PHASE-14 **build-ground-truth fallback**: Vitest (projenin gerçek test aracı, ortam-bağımsız) + `@anthropic-ai/sdk` mock (0-token mock seviyesinde garantili). **13/13 PASS (11 route node + 2 UI jsdom), kapsam-içi bug YOK, toplam gerçek API çağrısı = 0.** Kod-inceleme: sanitizasyon sırası [route.ts:21-24](src/app/api/chat/route.ts#L21-L24) apiKey-gate→503 · [:26-32](src/app/api/chat/route.ts#L26-L32) parse→400 · [:35-42](src/app/api/chat/route.ts#L35-L42) rol whitelist+trim+`slice(-12)` · [:44-46](src/app/api/chat/route.ts#L44-L46) sonda-user→400 · [:48](src/app/api/chat/route.ts#L48) `new Anthropic()` — **hepsi Anthropic kurulumu ÖNCESİNDE ✓**. Route (SDK mock): key-yok→503 (mock hiç kurulmadı); geçersiz-JSON/boş-`[]`/`messages`-yok/rol-enjeksiyonu-system/sonda-user-yok/whitespace → 6× 400 (mock hiç çağrılmadı, Anthropic öncesi red); geçerli-user→200 + mock çağrıldı (gerçek SDK asla); gömülü system+tool roller sıyrıldı yalnız temiz user gitti; 15 mesaj→`slice(-12)`=12; aşırı-uzun content→200 (max-byte cap YOK). Offline UI (gerçek `<Chatbot>` `#chat` inline, `Reveal` passthrough-mock, fetch 503): dürüst çevrimdışı metni (`chat.error` "…çevrimdışı…") + **sahte online-dot/presence YOK** (`/online/i`+`●` yok) + UI takılmaz (streaming reset, input etkin) + 2 deneme ikisi de 503 → 200-stream hiç. Harness (`tests/_verify-s7-*`) koşu sonrası silindi → git temiz, kaynak değişmedi. **Sahipli (record-not-fix):** per-mesaj max-byte uzunluk cap'i yok ("uzunluk" = min-length reject-empty + geçmiş-sayısı `slice(-12)`) → güvenlik-hardening adayı, 0-token kapsamı dışı + bloklayıcı değil → prd-review. Sıradaki adım: run-task (TASK-14.09, S9 adversarial/holistik).

...
**Son Güncelleme:** 2026-07-02 — run-task 9.09 (S9 adversarial/holistik) tamamlandı — **fazın son task'ı (9/9 ✅)**. `next build` **temiz** (37/37 SSG, log 0 error/warn/MISSING/deprecation); render `MISSING_MESSAGE` sweep **30 sayfa-locale → 0** (AR home dir=rtl teyit) = regresyon tabanı. **JS-kapalı SSG (curl, TR cookie):** 6/6 sayfa okunur — non-greedy strip görünür-metin 1272–4000 char, gerçek `<h1>` + nav ("← Ana sayfa"/6 iç link) + tek `<main>`, MM=0; LivingFlow client-only → markup'ta yok (dekoratif, beklenen). **Race (standalone Playwright `channel:'chrome'`+swiftshader, fresh prod :3100, 14/14):** WebGL sanity (webgl2+canvas=1, mode high → race anlamlı); **tema** 11 hızlı tık → `html.dark`==`localStorage`=="dark"==`aria-pressed` tutarlı (toggle DOM-class ground-truth; React state desync yok) + reload kalıcı; **dil** hızlı zincir en→de→ar→es→tr → final `lang==url==tr`+içerik render+MM yok, AR → `lang=ar`+`dir=rtl`+`/ar`; **scroll/anchor** 8'li anchor storm (60ms) → `#how` settle (top=0), native jump bottom6246↔top0 (**pin/scrub kilidi yok**), 12'li rastgele savurma sonrası 6 bölüm sağlam; tüm storm'larda **0 JS/ScrollTrigger hatası**. Ground-truth: served `/ar` h1 == disk `.next/server/app/ar.html` (fresh-build, stale-server yok). **Kapsam-içi bug yok, kaynak kod değişmedi** (TK7 triyaj: yeni kapsam-dışı kalem yok). Faz genel doğrulama özeti → verify-phase 9. loadavg run-anında 1.86 (düşük). **Sanitizasyon kod-inceleme (`route.ts`):** apiKey gate `:21-24` **ilk** (yoksa 503) → `req.json()` try/catch `:27-32` (parse hata 400) → sanitize filter `:35-42` (**rol whitelist** user/assistant + `content` string+`trim>0` + `slice(-12)` **geçmiş sınırı**) → sonda-user zorunlu `:44-46` (yoksa 400) → `new Anthropic()` **`:48`**: tüm red yolları **öncesinde**. **Dummy-key malformed batarya (curl, `ANTHROPIC_API_KEY` present-ama-geçersiz, fresh prod :3100):** geçersiz-JSON / `{}` / `[]` / not-array / trailing-assistant / **system-only rol-enjeksiyonu** / whitespace / non-string / empty-string / no-body → **10/10 400**, `new Anthropic()`'e ulaşmadan → sıfır çağrı; dummy key **present** olduğundan apiKey gate geçti (gerçekten malformed kısa-devre test edildi, key-gate değil). **Key-yok offline UI (standalone Playwright `channel:'chrome'`+swiftshader, `env -u ANTHROPIC_API_KEY`, `/proc/<pid>/environ` key **count=0** teyitli):** geçerli mesaj bile **503** "...is not configured" (apiKey gate sanitizasyondan **önce** → **TK4 kanıtı:** naif "key-yok+malformed→400" yanlış-negatif); 8/8 — başlangıç greeting görünür, **sahte '● online/canlı' presence dot YOK**, öneri tıkla→503→**dürüst offline metni** ("Asistan şu an çevrimdışı…"), kullanıcı balonu render, **Thinking göstergesi kalmadı** (UI takılmadı), input etkileşimli kaldı, ağ gözlemi tüm `/api/chat`=**503** + **200-stream HİÇ**. Stream-kopması `route.ts:69-77` try/catch/finally (clean fallback + close) + `Chatbot.tsx:38-42/:61-65` (!res.ok/catch→offline) kod-inceleme. **0-token:** gerçek key hiç kullanılmadı, happy-path koşulmadı, gözlenen tüm yanıt 400/503 → **toplam çağrı=0.** **Kapsam-içi bug yok, kaynak kod değişmedi.** Ölçüm nüansı (task-icra): dummy→no-key geçişinde stale `next-server` (cmdline `next-server` ≠ `next start` → pkill kaçırdı) portu tuttu → listening-PID ile yakalanıp `kill <PID>`+`setsid` temiz restart (memory stray-server disiplini). Bkz. Task Listesi 9.08 satırı.

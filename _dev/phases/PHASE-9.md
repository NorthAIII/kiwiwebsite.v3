...
**Son Güncelleme:** 2026-07-02 — run-task 9.04 (S8-Lighthouse): **6/6 dark kanonik Lighthouse a11y=100** (0 düşen audit) + **12/12 gerçek light/dark axe 0 Lighthouse-ilgili ihlal** (LH npx-cache axe-core 4.12.1, `localStorage` tema zorlama + themeOk teyit) → a11y=100 çift-tema mühürlü (iki-gate TK5 kapandı: 9.03 axe WCAG-AA + 9.04 Lighthouse structural). Perf korunan taban regresyonsuz: masaüstü perf 100/LCP 629ms/CLS 0 · mobil LCP 3171ms(≈3164 comparable)/FCP 1516ms/CLS 0 (perf 65/TBT 2000 = ağır-SwiftShader env anomalisi, kıyaslanamaz — memory). LCP elementleri değişmedi. Kapsam-içi bug yok, kaynak kod değişmedi; brief mobil perf açığı record-not-fix (TK7). · run-task 9.03 (S8-suite + S6-parite): `test:e2e` **52/52** axe WCAG-AA yeşil (home 2 + subpages 50, fail-on-regression) · `test` **7/7** Vitest yeşil (5-dil parite + smoke + umami) · CI run 28585690647 `fast`+`a11y` iki job `success` (HEAD 994ded9). Kapsam-içi bug yok, suite regresyonsuz. · run-task 9.02 (S5 + S6-render): 30/30 sayfa render-metin taraması. **S5:** "Crew OS" beklenen yüzeylerde (home 7×/showcase 5×/bülten 1×, 5 dil), "Bunker" 30/30 görünür metinde yok, yasak metafor/sahte-online/uydurma-sayı temiz. **S6-render:** 0 MISSING_MESSAGE, TR-leak yok (=TR%0), AR `dir=rtl`+Arapça glif teyit. Kapsam-içi bug yok, kaynak kod değişmedi. Sahipli (record-not-fix): 4 alt sayfa non-TR ar/de/es İngilizce-stale (=EN%87–93) → versiyon-sınırı çevirisi. · run-task 9.01 (S1): 30/30 route 200, redirect'ler 308, çıplak `/bulten`→404 (record-not-fix), `/tr`→307→`/` + bilinmeyen-locale→404 (beklenen); kapsam-içi bug yok.

...
**Son Güncelleme:** 2026-07-02 — run-task 9.07 (S2 tam TR yolculuğu + alt-sayfa çıkış/dönüş, curl+standalone Playwright): **14 kontrol — 14 ✓ · 0 ✗**. **Ana sayfa akış (curl+grep, `NEXT_LOCALE=tr`):** bölüm ID sırası tam `top→how→sectors→bunker→forum→contact`; çıkış href'leri mevcut (`/spor-salonu-yazilimi`×3, `/bunker-os`×2, `/vaka-calismalari`×1, 2 bülten×1); anchor CTA `#contact/#sectors/#top/#how/#bunker/#forum` hepsi mevcut bölüm ID'sine çözülüyor; **dead-`#`/boş-href yok** (Footer filter), **`/tr/` sızıntısı yok**, home+5 hedef **0 MISSING_MESSAGE**, 5 alt-sayfa **HTTP 200 + tek `<main>` + 39–47KB**. **Runtime (Playwright, full-motion, `channel:'chrome'`+swiftshader):** anchor scroll hedef `top≈0` (Hero ikincil→#sectors=0, birincil→#contact=52, Nav #how/#bunker/#forum=0), Lenis smooth-scroll doğru bölüme oturur (ScrollTrigger stabil). **4 alt-sayfa client-nav çıkış** — `window.__spaMarker` **korundu** (full-reload olsa silinirdi → client-nav kanıtı), hedef içerik bütün (h1 gerçek metin, `<main>`, 0 MISSING, txt 23–25K); **history-back dönüş** (App Router soft-nav, marker korundu) ana sayfayı **bozmadan** getirdi (url=`/`, 6/6 bölüm ID, hero h1 sağlam). **Kapsam-içi bug yok, kaynak kod değişmedi.** Faz 3 "anchor path-düşme" bu yüzeyde **yok** (hash-anchor path değiştirmez). **Ölçüm nüansı (task-icra):** `Nav→#forum` ilk-FAIL (foruma değil dibe kaymış) ölçüm artefaktıydı — Lenis `lerp:0.1` uzun smooth-scroll sabit-1400ms'de oturmadı + döngüde native `scrollTo(0,0)` Lenis-state'ini kirletti; çözüm hedef-başına taze reload + `settleScroll()` polling → `#forum` tam top=0/scrollY=3496 (Lenis smooth-scroll ölçümünde sabit-bekleme yerine settle-poll + izole reload; S9/9.09 hızlı-scroll race'e taşınır). Bkz. Task Listesi 9.07 satırı.

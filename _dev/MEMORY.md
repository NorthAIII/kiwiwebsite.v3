...
**Son Güncelleme:** 2026-07-16 — run-task 15.05: yeni [axe-offscreen-inline-contrast](memory/axe-offscreen-inline-contrast.md) eklendi (Teknik Tuzaklar) — ink-panel üstünde parlak küçük inline vurgu (`text-pulse-ink`) viewport-dışıyken axe zemini `<body>` canvas'ına düşürüp yanlış `color-contrast` fail verir; çözüm vurgu node'una panelle aynı renk `bg-ink` (görsel değişimsiz). TASK-15.06 fiyat bandı aynı deseni kullanacak.

...
**Son Güncelleme:** 2026-07-03 — run-task 12.03: Teknik Tuzaklar'a [axe-webgl-contrast-incomplete](memory/axe-webgl-contrast-incomplete.md) eklendi — axe `color-contrast` WebGL canvas arkasındaki metni ölçemez → `incomplete` (ihlal değil); otomatik gate kör, WebGL-arkası kontrast sertifikası craft görsel incelemedir.
...
- [next.config.ts `redirects()` `source`'u locale prefix'ini otomatik kapsamaz](memory/next-config-redirect-locale-prefix.md) — Config redirect `source` **literal** eşleşir → next-intl `as-needed` prefix'lerini (`/en`,`/ar`,`/de`,`/es`) kapsamaz. Tek `source:"/foo"` yazarsan çıplak `/foo`→308 ama `/en/foo`→**404** (ampirik: `/forum`→308, `/en/forum`→404). Doğru desen **iki açık giriş**: çıplak + `source:"/:locale(en|ar|de|es)/foo"` (`destination:"/:locale/…"`). `permanent:true`→308. Config redirect middleware'den ÖNCE (edge) çalışır → çift-redirect yok. Uyarı: aynı yolda fiziksel route klasörü kalırsa route 200 kazanır, redirect fire etmez → eski klasörü sil. (TASK-11.01, Faz 11.)

...
**Son Güncelleme:** 2026-07-02 — v0.2 production release: Ortam & Araç Notları'na [vercel-git-disconnect-deploy-tetiklenmez](memory/vercel-git-disconnect-deploy-tetiklenmez.md) eklendi — `main` push'u hiç deployment yaratmıyorsa Vercel Git bağlantısı kopmuş olabilir; yeniden bağlan + yeni (boş) commit push et (geriye dönük deploy yok). Canlı-kod teyidi immutable chunk 200 + string-in-HTML; `grep|head -1 &&` yanlış-pozitif tuzağı.

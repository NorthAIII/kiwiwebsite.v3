...
**Son Güncelleme:** 2026-09-11 — TASK-18.10: Süreç Disiplinleri'ne yeni kural — ziyaretçiye görünen UI etiketine i18n dışından (prompt/README/doküman) sabit adla atıf yapılmaz, işlevi betimlenir; hitap düzeyi de o dilin site kopyasıyla hizalanır (UAT 18 senaryo 28/29).
...
- [axe color-contrast: viewport-dışı küçük inline node zemini `<body>`'ye düşer](memory/axe-offscreen-inline-contrast.md) — İnk-panel (`bg-ink`) üstünde parlak renkli **küçük inline vurgu** (`<b class="text-pulse-ink">`, gerçek ≈11.5:1) axe'te **yanlış** `color-contrast` violation verir (`bgColor=#f7f6f1`, ~1.5:1, yalnız light). Neden: `scrollThrough` scroll-0 reset → alt-fold panel viewport-dışı → axe `elementsFromPoint` boş dönüp opak paneli çözemez, sayfa canvas'ına düşer (büyük `<p>` "incomplete" geçer, küçük inline node kesin-fail). **Çözüm (görsel değişimsiz):** aksan node'una **immediate backdrop ile aynı renk** opak zemin ver (doğrudan bant üstünde `bg-ink`, lifted kart üstünde kart rengi) → axe kendi opak zeminini ölçer, geçer. İki temada hem gerçek-panel hem body-fallback'i geçen tek aksan rengi yoktur; helper'ı değiştirme. Muted translücent (`/65`) metin "incomplete" kalır, dokunma. İki kez doğrulandı: aside pull-quote (15.05) + fiyat bandı eyebrow/₺/free-satır (15.06).
...
- [Üçüncü-parti model adı bozulabilir bağımlılıktır](memory/groq-model-emekliligi-runtime-404.md) — Groq `llama-3.3-70b-versatile`'ı research'teki "deprecated değil" damgasından ~7 hafta sonra **tamamen kaldırdı** (canlı 404 `model_not_found`). **Hiçbir yerel kapı yakalamaz:** model adı derlenmeyen bir string → `next build` temiz, Vitest yeşil, curl **200** döner (hata stream içindeki `try/catch` fallback metnine dönüşür). **Tek belirleyici kanıt `vercel logs <deployment>`.** Go-live duman testinde "200 geldi" yeterli değil — yanıt gövdesi fallback metni mi, bak. Uzun aradan sonra deploy ediyorsan model adını önce `/openai/v1/models` ile doğrula. Model adı değişimi mimari değişim değildir (`CHAT_MODEL` override deseni tam bunun için). Yeniden seçerken eleme kriterini (**dürüstlük konvansiyonu**) yeniden uygula — 18.07 marka mührü harness'i yeniden koşulabilir araçtır. **İkinci tuzak aynı dosyada:** `max_tokens` tavan değil **peşin rezervasyon** — Groq ücretsiz tier'da dakikalık çıktı kotasından istek anında düşer, 1024 istemek her çağrıyı 429 ile reddettirdi (→ 512). Bir canlı arızayı düzeltince "tamam" deme, **aynı yoldan tekrar doğrula**; arkasında ikinci sebep durabilir. (TASK-18.08.)
...
- [Vercel Git-disconnect → deploy tetiklenmez](memory/vercel-git-disconnect-deploy-tetiklenmez.md) — **`main`'e push (merge dahil) hiç deployment yaratmıyorsa** Vercel↔GitHub Git bağlantısı kopmuş olabilir (Deployments'ta sıfır satır — Error bile değil). Teşhis: Settings → Git "Connected Nm ago". Vercel geriye dönük deploy etmez → yeniden bağlan + **yeni** (boş) commit push et (`--allow-empty`). **Auth'suz deploy-teyidi (repo public):** GitHub `/repos/.../deployments` (Production+sha) + `/commits/<sha>/statuses` (`context=Vercel`,`state=success`) dashboard'suz "tetiklendi+tamamlandı" der. Canlı-kod teyidi: immutable chunk 200 + yeni-koda-özgü string HTML'de + `age:` düşük; **no-op/doc-merge'de** yeni-string yok → o merge'e özgü gözlemlenebilir fark diskriminatördür (18.01: orphan PNG 200→404). Shell tuzağı: `grep | head -1 &&` her zaman "VAR" basar (head boş girdide 0 döner) → `grep -c`/`grep -q` kullan. (v0.2 release 2026-07-02; auth'suz teyit + no-op diskriminatör TASK-18.01 2026-07-22.)
...
- **Ziyaretçiye görünen bir UI etiketine i18n dışından sabit adla atıf yapma — işlevini betimle.** Chatbot prompt'u, README, doküman gibi çeviri sistemine bağlı **olmayan** bir yüzeye site kopyasını (buton/menü adı) kopyalarsan iki yönlü bayatlar: etiket `messages/*.json`'da değişince atıf yalan olur (ziyaretçi sayfada olmayan adı arar — dürüstlük konvansiyonu), ve tek dilde yazıldığı için diğer dört dilde dil sızıntısı yaratır. Tek kaynak `messages/*.json`; dışarıdan atıf **betimleyici** yapılır ("sayfadaki ücretsiz keşif görüşmesi butonu"), tırnaklı sabit ad değil. (Chatbot SYSTEM_PROMPT'u Anthropic döneminden taşıdığı `"Book a call"` etiketiyle canlıda 5 dilde var olmayan bir butona yönlendirdi; aynı sınıf: bot/metin hitap düzeyi de o dildeki site kopyasıyla hizalanır — DE `Sie`, ES `tú`. UAT 18 senaryo 28/29, DECISIONS 2026-09-11.)
- [Runtime harness yazarken selector varsayma + şüpheyi belirleyici probe ile kapat](memory/runtime-harness-selector-teyidi.md) — **Standalone Playwright/runtime harness'i yazarken hedef komponentin etkileşim mekanizmasını kaynaktan teyit et** (DOM şeklini tahmin etme). Bu projede tekrar eden yanlış-FAIL kaynağı: LanguageSwitcher `<a href>` değil `router.replace` **butonu**; Chatbot floating değil inline `#chat` section; tema `html.dark`+localStorage (prefers-color-scheme değil, `emulateMedia` çevirmez). Harness "FAIL" verince önce **artefakt mı** diye sor (kör red yok), sonra gerçek-bug'a geç (kör onay yok) — ve şüpheyi **harness'ın kısıtını paylaşmayan bağımsız bir substratta** (gerçek middleware'li canlı site / tam-doküman navigasyon / gerçek UI affordance'ı / statik kaynak grep) koşulan **belirleyici probe** ile kapat; aynı harness içinde assertion gevşetmek kanıt değildir. Uygulama anı = runtime harness yazımı + beklenmedik sonuç anı (Faz 3, Faz 9, **Faz 17 S2/S3/S4/S9 — 5 şüphenin 5'i bu yöntemle kapandı**). [WebGL channel:'chrome'](memory/playwright-bundled-chromium-webgl-yok.md) ile aynı aile.

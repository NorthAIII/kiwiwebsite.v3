...
**Son Güncelleme:** 2026-07-02 — review-phase 11: Teknik Tuzaklar'a [next-config-redirect-locale-prefix](memory/next-config-redirect-locale-prefix.md) eklendi — `next.config.ts` redirect `source` locale prefix'ini kapsamaz; her redirect için iki-giriş deseni (çıplak + `/:locale(en|ar|de|es)/…`) şart (yoksa `/en/foo`→404).

...
**Son Güncelleme:** 2026-06-30 — run-task 6.04: Teknik Tuzaklar'a "Lighthouse Lantern render-timing körlüğü" pointer'ı eklendi (mobil Lantern-simüle skoru render-zamanlama lever'larını — opacity-gate/rIC deferral — göremez; TASK-6.04 L1+L2 birebir-delta kanıtı; perf bütçesi temiz doğrulaması gerçek-cihaz/field gerektirir).
...
- [Lighthouse Lantern render-timing körlüğü](memory/lighthouse-lantern-render-timing-korligi.md) — **Mobil Lighthouse skoru Lantern-simüle:** observed trace throttle'sız toplanıp 4× CPU *simüle* edilir → **render-zamanlama lever'ları (opacity-gate kaldırma, `requestIdleCallback` deferral) lab skorunda görünmez**, gerçek-cihaz-doğru olsalar bile. Kanıt (TASK-6.04): L1+L2 sonrası mobil LCP/TBT **birebir** değişmedi (observed `elementRenderDelay` 173↔173ms); rIC throttle'sız trace'te anında ateşler → Lantern erken-bloke simüle eder. "Delta yok" lab limiti olabilir, lever hatası değil — doğrula (commit + LCP elementi + observed renderDelay birebir). Simüle-LCP'yi gerçekten düşüren tek lab-lever = WebGL **iş yükünü** azaltmak (P2); perf bütçesinin temiz doğrulaması gerçek-cihaz/Vercel field gerektirir.

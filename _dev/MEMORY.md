...
**Son Güncelleme:** 2026-07-03 — run-task 13.04: [next-config-redirect-locale-prefix](memory/next-config-redirect-locale-prefix.md) güncellendi — `:slug*` opsiyonel gruba derlenir → çıplak `/forum`'u da eşler; ıraksak hedefte (çıplak→`/`, slug→`/bulten`) **çıplak giriş sırada önce** gelmeli (Faz 13). `/forum` gap kapandı.

...
**Son Güncelleme:** 2026-07-02 — **run-task 9.06 (S4-kontroller/kalıcılık) tamamlandı:** standalone Playwright (`channel:'chrome'`+swiftshader) 17 kontrol 17✓·0✗ — tema toggle→`html.dark`+localStorage+`themechange`+bg flip, reload kalıcı (early==final, FOUC yok), LivingFlow uniform canvas sameNode (remount yok→MutationObserver); dil-switcher path-koru (sub→EN, home→DE)+Escape/dış-tık/klavye; klavye-only 14 öğe yeşil focus-visible + odak kaybı yok. Kapsam-içi bug yok; Faz 3 anchor path-düşme burada YOK. Adım=task → sıradaki: **run-task TASK-9.07**.
...
**Adım:** task — sıradaki adım **run-task TASK-9.07** (S2 tam TR yolculuğu + alt-sayfa çıkış/dönüş — curl+Playwright client-nav; yeni oturum). 9.01 (S1) ✅, 9.02 (S5+S6-render) ✅, 9.03 (S8-suite+S6-parite) ✅, 9.04 (S8-Lighthouse) ✅, 9.05 (S3-degradasyon) ✅, 9.06 (S4-kontroller) ✅. Kapsam + Araştırma + Plan + verify-plan tamamlandı (`phases/PHASE-9.md`): 9 doğrulama task'ı (S1–S9 → TASK-9.01…9.09), suite-first hibrit metodoloji, TK1–TK7.
**İlerleme:** run-task 9.06 (S4-kontroller/kalıcılık, 2026-07-02) tamamlandı — standalone Playwright (`channel:'chrome'`+swiftshader) 17 kontrol 17✓·0✗: tema toggle (home+spor-salonu-yazilimi light↔dark) `html.dark`+`localStorage['theme']`+`themechange` event+bg flip; reload kalıcı (early.dark==final==ls, FOUC yok); LivingFlow uniform canvas **sameNode** (remount yok→FlowCanvas MutationObserver uniform çevirir); dil-switcher path-koru (`router.replace(pathname)`: sub TR→EN=`/en/spor-salonu-yazilimi`, home TR→DE=`/de`)+Escape/dış-tık/klavye kapanış; klavye-only 14 öğe yeşil focus-visible (rgb(31,122,61))+odak kaybı yok+klavye tema toggle. Kapsam-içi bug yok, kaynak kod değişmedi; Faz 3 anchor path-düşme bu yüzeyde YOK. Ölçüm nüansı: Tailwind v4 `transition-colors` outline-color'ı anime eder→reduced-motion ile ölçüldü. **Bekleyen versiyon-sonu aksiyonu:** v0.2 production release (tüm revize `main`'e ilk merge) — Umami canlı +1 (S9-10 Faz 7) o adımda kapanır (senaryo testi kod-tarafı varlığı S8'de doğrular, canlı panel değil). **Kapsam dışı (bilinçli açık):** brief mobil perf (gerçek-cihaz duvarı), TB-C npm audit, `/bunker-os`→`/crew-os` redirect + `/forum`→404 (görsel/SEO versiyonu), dil setini değiştirme (prd-review).
**Son Faz Dokümanı:** `phases/PHASE-9.md` (🔄 Devam ediyor — 9 task, 9.01–9.06 ✅). Faz 8 ✅ `phases/PHASE-8.md`.
...
**Task:** Sıradaki aktif task **TASK-9.07** (S2 — tam TR yolculuğu + alt-sayfa çıkış/dönüş: curl+Playwright client-nav, kopuk link/boş bölüm yok) — Faz 9, adım=task. 9.06 (S4-kontroller) ✅ archive'da; **run-task ile TASK-9.07**'den devam (yeni oturum).
**Durum:** Faz 9 (senaryo testi) 🔄 — adım=task. 9.01–9.06 ✅ (6/9). Faz 8 tüm task'ları ✅ (8.01→8.06) archive'da.
**İlerleme:** run-task 9.06 (S4-kontroller/kalıcılık) tamamlandı (2026-07-02): standalone Playwright (sistem Chrome + swiftshader) 17 kontrol 17✓·0✗ — tema toggle→html.dark+localStorage+themechange+bg flip, reload kalıcı (FOUC yok), LivingFlow uniform sameNode (remount yok); dil-switcher path-koru+Escape/dış-tık/klavye; klavye-only 14 öğe yeşil focus-visible+odak kaybı yok. Kapsam-içi bug yok, kaynak kod değişmedi; Faz 3 anchor path-düşme burada YOK.

---

## Task Durumu (Aktif Faz)

> Faz 9 (senaryo testi) 🔄 — 9 task hazır, 9.01–9.06 ✅ (6/9). Faz 8 ✅ (8.01-8.06 archive'da, detay `phases/PHASE-8.md`); Faz 7 (Umami E1) ✅; Faz 6 ✅ (6.06 ❌ iptal); Faz 5 ✅; Faz 4 ✅ — hepsi archive'da.
...
| 9.07 | TASK-9.07 | ⬜ Bekliyor | S2 — tam TR yolculuğu + alt-sayfa çıkış/dönüş (curl+Playwright) ← sıradaki |
...
**Aktif Task:** Sıradaki TASK-9.07 (S2 tam TR yolculuğu + alt-sayfa çıkış/dönüş — curl+Playwright) — Faz 9, adım=task. 9.01–9.06 ✅ (6/9). Sıradaki: run-task TASK-9.07. Bekleyen: v0.2 production release (Umami canlı +1 orada kapanır).
**Aktif Faz:** 9 (v0.2 senaryo testi) · adım=task — 9.01–9.06 ✅ (6/9); Faz 8 ✅ (8.01→8.06; review tamam); Faz 7 ✅; Faz 6 ✅; Faz 5 ✅; Faz 4 ✅; Aktif Versiyon v0.2, Versiyon Sonu Durumu: senaryo_testi
...
**Son Güncelleme:** 2026-07-02 — run-task 9.06 (S4-kontroller/kalıcılık): standalone Playwright (`channel:'chrome'`+swiftshader) 17 kontrol 17✓·0✗ — tema toggle→`html.dark`+localStorage+`themechange`+bg flip, reload kalıcı (early==final, FOUC yok), LivingFlow uniform canvas sameNode (remount yok→MutationObserver); dil-switcher path-koru (sub→EN, home→DE)+Escape/dış-tık/klavye; klavye-only 14 öğe yeşil focus-visible+odak kaybı yok. Kapsam-içi bug yok, kaynak kod değişmedi; Faz 3 anchor path-düşme burada YOK. Adım=task. Sıradaki: run-task TASK-9.07.

...
**Aktif Task:** **TASK-17.05** ⬜ Bekliyor (sıradaki `/devflow:run-task`; runtime C katmanı — S4 kontroller & kalıcılık). TASK-17.01 ✅ (S1) · TASK-17.02 ✅ (S5+S6-render+Alpfit render) · TASK-17.03 ✅ (S8-suite+S6-parite) · TASK-17.04 ✅ (S3 degradasyon + BULGU-S3). 4 task ⬜ (TASK-17.05–17.08). **⚠️ Açık takipler (regresyon değil):** branch→main merge bekliyor (gym PNG, versiyon-sonu finalizasyona) · canlı `ANTHROPIC_API_KEY` env YOK (`/api/chat` 503 → chatbot "offline"; kullanıcı aksiyonu) · non-TR alpfit stale-TR (prd-review; 17.02/17.03'te yapısal parite teyitli, kopukluk yok) · **BULGU-S3** (17.04): alt-sayfa hero'ları `high` masaüstünde animasyonlu Living Flow yok (yalnız base-wash; crew-os ile birebir → regresyon değil, craft nüansı; **kullanıcı kararı** — fix task vs prd-review).

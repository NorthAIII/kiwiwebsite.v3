...
| Faz 11 ✅ (v0.3 URL taksonomisi/SEO) → sonraki içerik fazı | 2026-07-02 | Milestone karşılandı: public `/crew-os` 5-locale SSG + kalıcı 308 redirect (çıplak+5-locale, locale gap yok) + namespace 5-dil senkron (0 MISSING_MESSAGE) + sitemap/iç link temiz (çift-redirect yok); taksonomi (DECISIONS 2026-06-27) son açık ucu kapandı; UAT 13/13, kalite 8 eksen (7 ✅ + 1 N/A); kod-adı tanımlayıcıları iç-ad korundu; kayıtlı açık: latent canonical=`/` + `/forum` locale gap (gelecek SEO fazı); guardrail regresyonsuz; Versiyon Sonu Durumu içerik_fazları (değişmez); sıradaki = v0.3 sonraki içerik fazı — B1 Living Flow nabız (discuss-phase 12 promote eder) |
| Faz 12 girildi (v0.3 Living Flow nabız kapsamı B1) | 2026-07-02 | discuss-phase 12: içerik_fazları (normal döngü); karar-gate'li/imza-riskli faz; kapsam kontrollü aşağı-taşıma → sürekli soluk iplik + desktop-öncelik (mobil korunur) + bölüme-uyarlanan opaklık; gate: kontrast=100/perf tabanı/craft korunamıyorsa iptal-kaydet (P2 emsali); yeni i18n anahtarı yok; mobil aşağı-taşıma + /crew-os/Alpfit/404 + Hero çekirdek efekti kapsam dışı; Sıradaki Fazlar'dan mezun (v0.3 içerik konusu bitti), tabloya 🔄; sıradaki adım research-phase 12 |
| Faz 12 ✅ (v0.3 Living Flow nabız kapsamı B1) → versiyon-sonu | 2026-07-03 | Karar-gate **uygula-onayla** (üç gate geçti: a11y=100 çift-tema full-motion / desktop perf 100·CLS 0 / craft — light bleed `--flow-veil` token'ıyla çözüldü); milestone ✓ (imza güçlendi, fallback/kontrast/perf tabanı regresyonsuz); tek WebGL context (shared `useFlowMode`) + emergent adaptif veil (0 bölüm-dosyası dokunuşu); UAT 16/16, 8 kalite ekseni (7 ✅ + 1 ⚠️ test kapsamı boşluğu, kayıtlı); yeni bağımlılık/i18n anahtarı yok; kayıtlı açık: full-motion invariant tohumu + Faz 11 SEO açıkları; v0.3 içerik fazları (10,11,12) tamam; Versiyon Sonu Durumu içerik_fazları (değişmez); sıradaki = versiyon-sonu teknik borç fazı (discuss-phase 13 içerik_fazları→teknik_borç promote eder) |
| Faz 13 girildi (v0.3 versiyon-sonu teknik borç — SEO-metadata hijyeni) | 2026-07-03 | discuss-phase 13: Adım 0 versiyon-sonu tespiti `içerik_fazları`→`teknik_borç` damgalandı (v0.3 içerik fazları ✅ + Aktif Faz/Adım dolu); kullanıcı "borç listesi tam" onayı; kapsam TB-1 (alt-sayfa self-canonical + 5-locale hreflang alternates, kalıcılık) + TB-2 (`/forum` locale gap + **tüm** config redirect denetimi, kök çözüm) + hafif regresyon tohumu (WebGL-flaky değil, kümülatif test); TB-3 (full-motion tohumu) / TB-4 (logical-ok RTL) / TB-5 (npm audit) kayıtlı sahipli açık; B grubu (non-TR tazelik / AR-dil stratejisi / brief mobil perf) → prd-review; dinamik faz doğrudan tabloya 🔄; sıradaki adım research-phase 13 |
...
| Faz 15 ✅ (v0.4 Alpfit Plus ürün vitrini) → versiyon-sonu | 2026-07-16 | 7 task (15.01–15.07) + UAT 16/16; 0 kapsam-içi bug, 0 düzeltme task'ı; sayfa artifact düzenine port (9 bölüm, `components/alpfit/` 8 bileşen + CSS Module mockup + `alpfit` 5-dil 133-leaf namespace + tek `--color-surface` token); imza Living Flow + dürüstlük 4/4 korundu; kalite 8 eksen ✅; guardrail (a11y=100 çift-tema/CLS=0/i18n parite/marka sesi/reduced-motion) regresyonsuz; orphan Gym+`next/image` düştü; v0.4 içerik fazları (15) tamam; Versiyon Sonu Durumu içerik_fazları (değişmez); sıradaki = versiyon-sonu teknik borç fazı (discuss-phase 16 içerik_fazları→teknik_borç promote eder). Sahipli: non-TR stale · gym png disk hijyeni · canlı env |
| Faz 16 girildi (v0.4 versiyon-sonu teknik borç) + TR release | 2026-07-16 | discuss-phase 16: Adım 0 versiyon-sonu tespiti `içerik_fazları`→`teknik_borç` damgalandı (v0.4 AP1/AP2/AP3 ✅ + Aktif Faz/Adım dolu); kapsam gym PNG disk hijyeni (TB-D1) + npm audit (TB-D2, kullanıcı seçimi); non-TR çeviri + full-motion test tohumu kapsam-dışı. **Kullanıcı önceliğiyle v0.4 TR production release yapıldı** (App Store lansman aciliyeti): `revize/alpfit-plus`→`main` temiz ff (16 commit), build exit 0, canlı duman testi ✓ → canlı `f173234`; non-TR stale-TR (ertelendi), chatbot env açık takip; `docs/RELEASE-v0.4.md`. Dinamik faz doğrudan tabloya 🔄; sıradaki adım research-phase 16 (kalan teknik borç task'ları) |
| Faz 16 ✅ (Teknik Borç + TR release) → senaryo testi | 2026-07-17 | TB-D1 (gym PNG hijyeni, TASK-16.01) + TB-D2 (npm audit kabul+kayıt, sömürülemez build-zamanı postcss) + REL (v0.4 TR canlı `f173234`) tamam; UAT 11/11, kalite 8 eksen ✅, 0 düzeltme task'ı; review'da M2:126/127 doküman-drift kapandı; guardrail (a11y=100 çift-tema/perf tabanı/CLS≈0/i18n parite) regresyonsuz; Versiyon Sonu Durumu → senaryo_testi (discuss-phase 17 promote eder); sahipli: non-TR stale · branch→main merge · canlı env |
| Faz 17 girildi (v0.4 versiyon-sonu senaryo testi) | 2026-07-17 | discuss-phase 17: Adım 0b (Versiyon Sonu Durumu zaten senaryo_testi, review-phase 16 damgaladı); kapsam ana sayfa + 5 alt sayfa uçtan-uca (Faz 14 çıtası korundu), v0.4 delta odağı = **Alpfit Plus ürün vitrini** (9 bölüm SSG render + saf CSS/SVG PhoneMockups + `alpfit` 133-leaf 5-dil parite + before/after Living Flow + dürüstlük 4/4); **canlı katman = branch/build + hafif canlı duman** (test-what's-live, v0.4 canlıda `f173234`); TR birincil + non-TR tutarlılık (alpfit stale-TR görünür kopukluk yok), chatbot 0-token, keşfet+kaydet+triyaj, otonom; S1–S9 kataloğu onaylandı; branch→main merge (gym PNG) versiyon-sonu finalizasyona (kapsam dışı); dinamik faz doğrudan tabloya 🔄; sıradaki adım research-phase 17 |
...
**Son Güncelleme:** 2026-07-17 — discuss-phase 17: **Faz 17 (v0.4 versiyon-sonu senaryo testi) girildi** (🔄). Kapsam ana sayfa + 5 alt sayfa uçtan-uca (Faz 14 çıtası, Alpfit Plus delta odağı); branch/build + canlı duman (test-what's-live). Faz Durumu tablosuna eklendi; Sıradaki Fazlar boşaldı (v0.4 son sabit fazı girildi). Sıradaki adım `/devflow:research-phase 17`.
